...
def main(state, freq):
    ...
    results_file_path = SAVE_PATH / 'results' /\
        'EFS_NoGamma_{}_{}_{}_{:.2f}.mat'.format(state, freq, WINDOW, OVERLAP)
    if not path(results_file_path).isfile():
        ...
        for train_subjects, test_subjects in sl2go.split(final_data, lil_labels, lil_groups):
            ...
            best_elecs.append(CHANNEL_NAMES[list(best_idx)])
            best_scores.append(f_select.best_score_)

            test_clf = LDA()
            test_clf.fit(X_feature[:, best_idx], labels)
            y_classif = [np.array([label]*X_classif[i].shape[1])
...
if __name__ == '__main__':
    ...
    for state, freq in product(STATE_LIST, FREQ_DICT.keys()):
        main(state, freq)
