...
def cospectrum(X, window=128, overlap=0.75, fmin=None, fmax=None, fs=None):
    ...
    fdata = numpy.zeros((*map(int, (number_windows, Ne, number_freqs))),
                        dtype=complex)
    ...
    # Efficiently compute the matrix product of fdata.conj().T by fdata for each frequency
    S = numpy.einsum('abc,adc->bdc',fdata.conj(),fdata) / number_windows
